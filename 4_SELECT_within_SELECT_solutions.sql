-- These are my solutions of the More JOIN operations of SQLZoo
...
SELECT name, CONCAT(ROUND(100.0 *
...
WHERE name = (SELECT name FROM world
  WHERE x.continent = continent LIMIT 1);
...
WHERE population / 3 >= ALL(SELECT population
  FROM world AS y WHERE x.continent = y.continent AND x.name != y.name);
