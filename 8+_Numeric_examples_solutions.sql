-- These are my solutions of the NSS Tutorial Section of SQLZoo
...
FROM (SELECT x.institution, SUM(x.sample) AS total_sample
  FROM nss AS x
  WHERE x.institution LIKE '%Manchester%'
  AND x.question = 'Q01'
  GROUP BY x.institution) AS total
JOIN (SELECT y.institution, y.sample AS comp_sample
  FROM nss AS y
  WHERE y.institution LIKE '%Manchester%'
  AND y.question = 'Q01'
  AND y.subject = '(8) Computer Science'
  GROUP BY y.institution, y.sample) AS comp_stu
